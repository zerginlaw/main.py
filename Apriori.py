...
def get_candidate(Fk, K):       #求第k次候选集
    ...
    for i in range(len(Fk)):
        for j in range(i+1, len(Fk)):
            ...
            if L1 == L2:
                if K > 2:
                #第二次求候选集，不需要进行减枝，因为第一次候选集都是单元素，且已经减枝了，组合为双元素肯定不会出现不满足支持度的元素
...
dataset = result       #装入数据 二维列表
F, sup_data = Apriori(dataset, min_support = 17)   #最小支持度设置为2
